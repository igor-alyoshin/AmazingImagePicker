package com.widget.amazingimagepicker.superslim;
...
public class LayoutManager extends RecyclerView.LayoutManager {
    ...
    //TODO
    public int findFirstCompletelyVisibleNoHeaderItemPosition() {
        View firstVisibleView = null;
        SectionData sd;
        //if (getAnchorAtStart().getLayoutParams().getFirstPosition())
        for (int i = 0; i < getChildCount() - 1; i++) {
            sd = new SectionData(this, getChildAt(0));
            final SectionLayoutManager slm = getSlm(sd);

            firstVisibleView = slm.getFirstCompletelyVisibleView(sd.firstPosition, true);
            if (firstVisibleView != null) {
                LayoutParams lp = (LayoutParams) firstVisibleView.getLayoutParams();
                if (!lp.isHeader) {
                    break;
                }
            }
...
//            attachView(header, findLastIndexForSection(sd.firstPosition) + 1);
    ...
    public static class LayoutParams extends RecyclerView.LayoutParams {
        ...
        public static final int DEFAULT_HEADER_DISPLAY = HEADER_INLINE | HEADER_STICKY;
